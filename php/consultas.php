		...
		case 'agendasAdministrador':
			...
			# code...
				...
				if ($resultado = mysqli_query($conn,$sql)) {
					...
					$sql = "INSERT INTO `agenda` (`idAgenda`, `nombreAgenda`, `comentario`, `idUsuario`) VALUES (NULL, '$nombreAgenda', '$comentario', '$idUsuario')";
		...
		case 'registroAdministrador':
			...
			# code...
				...
				if ($resultado = mysqli_query($conn,$sql)) {
					...
					$sql = "SELECT cultivo.nombreCultivo,cultivo.descripcion,cultivo.tipoSemilla,cultivo.procedimiento,cultivo.comoSembrar FROM cultivo WHERE cultivo.idRegion='$idRegion'";
		...
		case 'actualizarCultivo':
			# code...
				...
				if ($resultado = mysqli_query($conn,$sql)) {
					...
					$sql = "UPDATE `cultivo` SET `nombreCultivo` = 'jhghjgjhj', `descripcion` = 'jghjgjhg', `tipoSemilla` = 'j', `procedimiento` = 'j#txtProcedimientoNuevo', `comoSembrar` = 'jjhgj', `idRegion` = '1', `idUsuario` = '898' WHERE `cultivo`.`idCultivo` = 6')";
					if($resultado = mysqli_query($conn,$sql)){
